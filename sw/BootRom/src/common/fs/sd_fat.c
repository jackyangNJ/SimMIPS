...
int list_root_files(uint8_t* buffer) {
    ...
    uint32_t sectorsPerFat32 = (fbs.bpb.sectorsPerFat32High << 16) + fbs.bpb.sectorsPerFat32Low;
    uint32_t dataRegionSectorAddr = fbs.bpb.reservedSectorCount + fbs.bpb.fatCount * sectorsPerFat32 + firstPartitionSector;
    //sector addr for Root Directory
    uint32_t fat32RootCluster = (fbs.bpb.fat32RootClusterHigh << 16) + fbs.bpb.fat32RootClusterLow;
    uint32_t rootDirSectorAddr = dataRegionSectorAddr + (fat32RootCluster - 2) * fbs.bpb.sectorsPerCluster;
