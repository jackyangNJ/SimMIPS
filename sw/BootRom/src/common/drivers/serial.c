...
void serial_init(void) {
    //disable the uart device
    out_byte(SERIAL_PORT + 1, 0x00);
    //get access to divisor latches
    out_byte(SERIAL_PORT + 3, 0x80);
    //write value to divisor latches to determine the bard rate
    //for 100MHz clock, current uart baud is 115200.
    //Divisor = 100M /16 / 115200;
    out_byte(SERIAL_PORT + 0, 54);
    out_byte(SERIAL_PORT + 1, 0x00);
    //config  8-bit mode
    out_byte(SERIAL_PORT + 3, 0x03);
    //clear fifo and set the fifo trigger level
